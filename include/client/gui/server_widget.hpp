...
public slots:
  ...
  void showContextBuild(const QPoint& pos)
  {
    ...
    if (selected_action == exe_action)
    {
      ...
      {
        ServerErrorBox* error = new ServerErrorBox(std::string("Failure to start container"));
        error->exec();
      }
    }
    ...
    {
      std::shared_ptr<RoboClient> c = clients_[last_server_name_];
      c->deleteRoboContainer(std::string(container_name_table_wgt->item(item_row, exe_del_info_col)->text().toUtf8().constData())),
    }
  }
